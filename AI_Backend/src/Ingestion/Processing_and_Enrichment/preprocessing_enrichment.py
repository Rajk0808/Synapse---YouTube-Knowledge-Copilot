...
from AI_Backend.src.Ingestion.Processing_and_Enrichment.artifacts import Artifacts
from AI_Backend.src.Ingestion.Processing_and_Enrichment.cleaning import Cleaning
from AI_Backend.src.Ingestion.Processing_and_Enrichment.chunk_transcript import ChunkTranscript
from AI_Backend.src.Ingestion.Processing_and_Enrichment.embeddings import Embeddings
from AI_Backend.src.Ingestion.Processing_and_Enrichment.tagging import Tagging
...
class preprocessing_enrichment:
    ...
    def invoke(self, data: dict) -> dict:
        ...
        result = self.chunk_transcript.invoke(cleaned_data)
        #enriched_data = self.embeddings.invoke(chunked_data)    
        #tagged_data =  self.tagging.invoke(chunked_data)
        #result = self.artifacts.invoke(chunked_data)
